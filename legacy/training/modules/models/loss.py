...
class EnergyLoss(nn.Module):
    ...
    def forward(self, output, target):
        energy_target = torch.sum(torch.pow(target, 2))
        energy_output = torch.sum(torch.pow(output, 2))
        loss = self.weight * torch.pow(torch.add(energy_target, -energy_output), 2)
        return loss
...
class LossWrapper(nn.Module):
    def __init__(self, losses={'ESR': 0.75, 'DC': 0.25}, filter_cfs=None):
        ...
        loss_dict = {'ESR': ESRLoss(), 'DC': DCLoss(), 'EnergyLoss': EnergyLoss()}
