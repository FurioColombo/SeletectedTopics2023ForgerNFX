...
def train_epoch(model: lstm_model.SimpleLSTM,  # torch.nn.Module,
...
def train_epoch_interval(model: torch.nn.Module,  # torch.nn.Module,
                         dataloader: DataLoader,
                         loss_functions,
                         optimiser,
                         device='cpu',
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        if type(model) is conv_model.SimpleConv1d:
        ...
        if sub_batch_seq_len > 0:
            ...
            available_sub_batches = 1
            sub_batch_seq_len = int(inputs.shape[1]) - 1

        start_sample = warm_up_len
        end_sample = warm_up_len + sub_batch_seq_len
        if end_sample >= int(inputs.shape[1]):
            end_sample = int(inputs.shape[1]) - 1
